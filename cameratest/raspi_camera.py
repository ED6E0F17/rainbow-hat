...
TIMEOUT = 1      # Camera may need 2 seconds to start up
...
poweroffcommand = [COMMANDSEND, SERIALNUM, CMD_POWERSAVE, 3, 0, 1, 0] # framebuffer off
poweroncommand = [COMMANDSEND, SERIALNUM, CMD_POWERSAVE, 3, 0, 0, 0] # framebuffer on
videooffcommand = [COMMANDSEND, SERIALNUM, CMD_VIDEOOFF, 1, 0 ] #  stop analog out
...
def lowpower():
	cmd = ''.join( map( chr, poweroffcommand ))
	s.write(cmd)
	reply = s.read(5)
	r = list(reply)
	if( checkreply( r, CMD_POWERSAVE) and r[3] == chr(0x0)):
		return True
	return False


def videooff():
	cmd = ''.join( map( chr, videooffcommand ))
	s.write(cmd)
	reply = s.read(5)
	r = list(reply)
	if( checkreply( r, CMD_VIDEOOFF) and r[3] == chr(0x0)):
		return True
	return False
...
if( not lowpower() ):
	print "framebuffer not disabled"
